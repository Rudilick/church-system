...
export default function AccountingPage() {
  ...
  useEffect(() => {
    deptsApi.list().then(r => setDepts(r.data))
  }, [])
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        ...
        await expensesApi.add({ ...form, amount: Number(form.amount), department_id: form.department_id || null })
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.deptTabs}>
        <button
          className={`${styles.deptTab} ${activeDept === null ? styles.deptTabActive : ''}`}
          onClick={() => setActiveDept(null)}
        >전체</button>
        {depts.map(d => (
          <button
            key={d.id}
            className={`${styles.deptTab} ${activeDept === d.id ? styles.deptTabActive : ''}`}
